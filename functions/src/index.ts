...
export const backupTripToNotion = onCall(
  ...
  async (request) => {
    ...
    const staticCars: any[]    = Array.isArray(tripData.staticCars)
      ? tripData.staticCars
    ...
    const isSettlementRecord = (e: any) => e?.category === 'settlement';
    const totalTWD = expensesSnap.docs.reduce((sum, d) => {
      const e = d.data();
      if (isSettlementRecord(e)) return sum;
    ...
    if (staticCars.length) {
      staticCars.forEach(c => {
        ...
        if (c.notes) blocks.push(...textBlocks(`備註：${c.notes}`));
      });
    } else {
    ...
    if (allJournals.length) {
      allJournals.forEach(j => {
        ...
        if (body) blocks.push(...textBlocks(body));
        else blocks.push(para('（無內文）'));
        const photoCount = Array.isArray(j.photos) ? j.photos.length : 0;
        if (photoCount) blocks.push(para(`📷 附 ${photoCount} 張照片（請至 app 查看）`));
      });
    } else {
